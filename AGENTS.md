...
Bun only (`bun`, `bunx`, `bun run`); the lockfile is `bun.lock`.
...
- All API responses use the `formatEntity()` / `formatErrorEntity()` helpers (`src/lib/utils/formatEntity.ts`)
...
- Don't test API route files (`route.ts`) or page files (`page.tsx`) directly
