...
interface VoiceConfigurationOptions {
  accessToken: string;
  systemPrompt: string;
  interview: Entity<InterviewWithJob> | undefined | null;
}

export function useVoiceConfiguration({
  accessToken,
  systemPrompt,
  interview,
  ...
  // Create session context from interview data
...
5. Return to any key questions that weren't fully answered before concluding the interview`;
  ...
  return {
    ...
    sessionContext,
    sessionSettings,
    isConfigured: !!accessToken && !!configId && !!systemPrompt,
  };
}
