      ...
      .select({
        ...
        hasVoted: currentUserId
          ? sql<boolean>`EXISTS (
              ...
              AND ${featureRequestLikes.userId} = ${currentUserId}
            )`.as("has_voted")
...
export async function POST(request: NextRequest) {
  logger.info("POST request received at /api/feature-requests");
  const { userId: clerkUserId } = getAuth(request);
  if (!clerkUserId) {
    logger.warn("Unauthorized access attempt to POST /api/feature-requests");
