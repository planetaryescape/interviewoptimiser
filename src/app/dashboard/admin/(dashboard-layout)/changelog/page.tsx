...
import type { Changelog, NewChangelog } from "~/db/schema";
  ...
  return (
    <section className="h-full grid grid-rows-[auto_1fr_auto] overflow-auto">
      <div className="flex justify-between items-center mb-4 row-span-1">
        <h2 className="text-2xl font-semibold text-foreground">Admin Changelog</h2>
      </div>

      {/* Add new changelog form */}
      <div className="mb-8 space-y-4 bg-card text-card-foreground p-4 rounded-lg shadow">
        <h3 className="text-lg font-semibold mb-4 text-foreground">Add New Changelog</h3>
        <Input
        ...
        {SimpleMDEComponent && (
          <SimpleMDEComponent
            ...
            onChange={(value) => setNewChangelog({ ...newChangelog, content: value })}
            options={simpleMDEOptions}
          />
        )}
        <Button variant="outline" onClick={() => addChangelogMutation.mutate(newChangelog)}>
          Add Changelog
        </Button>
      </div>
      ...
      {/* Existing changelogs */}
      <div>
        <h2 className="text-2xl font-semibold mb-4 text-foreground">Existing Changelogs</h2>
        {changelogs?.data.map((changelog) => (
          <div key={changelog.sys.id} className="bg-card p-6 rounded-lg shadow mb-4">
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-xl font-semibold text-foreground">{changelog.data.title}</h3>
            ...
            <p className="text-muted-foreground">Likes: {changelog.data.likes}</p>
          </div>
        ))}
      </div>
    </section>
